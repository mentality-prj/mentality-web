...
Dzvin.co використовує **Zitadel** як OIDC-провайдер із потоком **Authorization Code + PKCE**. Фронтенд ніколи не працює з `client_secret` — обмін токенів відбувається через серверний API-маршрут.
