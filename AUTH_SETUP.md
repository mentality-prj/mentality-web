...
# Backend JWT Secret - must match the secret on the NestJS backend
NEXT_PUBLIC_JWT_SECRET="your-backend-jwt-secret"
...
# For NEXT_PUBLIC_JWT_SECRET (must match the backend!)
# Use the same secret as on the NestJS backend
```
...
1. Verify that `NEXT_PUBLIC_JWT_SECRET` matches backend `JWT_SECRET`
2. Check that you send the correct token (`idToken` or `accessToken`)
3. Use Google Token Verification API on the backend
