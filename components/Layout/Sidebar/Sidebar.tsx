...
const Sidebar = async ({ menu, type }: { menu: SidebarMenuItemType[]; type?: SidebarMenuType }) => {
  // const locale = await getLocale() TODO: temporarily hide
  // const t = await getTranslations({ locale, namespace: 'components.AvatarMenu' }) TODO: temporarily hide
  ...
  return (
    <aside className={`flex min-w-64 flex-col rounded-l-3xl ${type === 'admin' ? 'text-white' : 'pl-6'}`}>
      ...
      {/* {bottomSidebarMenu} TODO: temporarily hide */}
    </aside>
  )
}
