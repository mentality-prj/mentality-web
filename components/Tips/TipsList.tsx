...
export default function TipsList({ fetchUnpublished = false, renderTools, reloadTrigger }: Props) {
      ...
      <ul className="grid grid-cols-1 items-stretch gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((a) => (
          <li key={String(a.id)} className="h-full flex-1">
            <TipCard item={a} tools={renderTools ? renderTools(a, removeItem) : undefined} />
          </li>
        ))}
      </ul>
