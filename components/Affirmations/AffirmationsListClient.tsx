...
export default function AffirmationsListClient({ fetchUnpublished = false, renderTools, reloadTrigger }: Props) {
        ...
        {items.map((a) => {
          ...
          return (
            <li key={String(a.id)} className="h-full flex-1">
              <AffirmationCard item={a} tools={renderTools ? renderTools(a, removeItem) : tools} />
            </li>
          )
        })}
      </ul>
